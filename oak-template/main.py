            ...
            try:
                ...
                if det is not None and depth_msg is not None:
                    ...
                    for d in det.detections:
                        x1_px, y1_px = int(d.xmin * w), int(d.ymin * h)
                        x2_px, y2_px = int(d.xmax * w), int(d.ymax * h)
                        x1_px, y1_px = max(0, x1_px), max(0, y1_px)
                        x2_px, y2_px = min(w - 1, x2_px), min(h - 1, y2_px)
                        if x2_px <= x1_px or y2_px <= y1_px:
                            continue

                        roi = depth_frame[y1_px:y2_px, x1_px:x2_px]
                        if roi.size < 10:  # skip tiny ROIs
                            continue

                        distance_m = roi.mean() / 1000.0  # mm → meters
